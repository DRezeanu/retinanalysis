...
def matlab_style_gauss2D(
    ...
    h = torch.exp(
        -(a * grid_rows**2 + 2 * b * grid_rows * grid_cols + c * grid_cols**2)
    )
...
class ParametrizedSpatialFilters(torch.nn.Module):
    def __init__(
        ...
        **d_filt_params,
        ...
        self.c_row_sigmas = self._to_param(
            d_filt_params["c_row_sigmas"], "c_row_sigmas"
        )
        self.c_col_sigmas = self._to_param(
            d_filt_params["c_col_sigmas"], "c_col_sigmas"
        )
        self.s_row_sigmas = self._to_param(
            d_filt_params["s_row_sigmas"], "s_row_sigmas"
        )
        self.s_col_sigmas = self._to_param(
            d_filt_params["s_col_sigmas"], "s_col_sigmas"
        )
    ...
    def get_ells(self, sd_mult=1.0, **kwargs):
        ...
        for i in range(self.n_cells):
            ...
            ell = Ellipse(
                xy=(self.col_coords[i].item(), self.row_coords[i].item()),
                width=2 * self.c_col_sigmas[i].item() * sd_mult,
                height=2 * self.c_row_sigmas[i].item() * sd_mult,
                angle=self.thetas[i].item(),
                **kwargs,
            )
            ells.append(ell)
    ...
    def compute_filter(self) -> torch.Tensor:
        filter = torch.zeros(
            (self.n_cells, self.n_ht, self.n_wt),
            device=self.device,
            dtype=torch.float32,
        )

        vec_gauss_fn = torch.vmap(
            matlab_style_gauss2D, in_dims=(0, 0, 0, 0, 0, None, None)
        )
        ...
        # Normalize by peak
        peaks = torch.max(torch.abs(filter).reshape(self.n_cells, -1), dim=1)[0].view(
            -1, 1, 1
        )
        peaks[peaks == 0] = 1
        filter = filter / peaks
    ...
    def forward(self, x: torch.Tensor):
        ...
        if len(x.shape) != 4:
            raise ValueError(
                f"Input x must be of shape [n_imgs, n_channels, n_ht, n_wt], but got {x.shape}"
            )
...
class Spatial_DoG(torch.nn.Module):
    def __init__(self, input_spatial_dims, device, d_init_params):
        ...
        self.parametrized_filter = ParametrizedSpatialFilters(
            input_spatial_dims=input_spatial_dims, device=device, **d_init_params
        )
    ...
    def constrain(self):
        with torch.no_grad():
            self.parametrized_filter.s_amps.data = torch.clamp(
...
def fit_model_params(
    ...
    for epoch in trange(n_total_epochs):
        ...
        if epochs_no_improve >= n_patience:
            print(
                f"Early stopping at epoch {epoch + 1} due to no improvement for {n_patience} epochs."
            )
            break

    d_track = {
        "train_loss": np.array(ls_train_loss),
        "ls_state_dicts": ls_state_dicts,
        "grad_norms": np.array(ls_grad_norms),
    }
...
def plot_spatial_dog_performance(
    ...
    for i_batch in range(n_plots):
        ...
        row_coords = (
            model.parametrized_filter.row_coords[i_start:i_end].detach().cpu().numpy()
        )
        col_coords = (
        ...
        if str_save_dir is not None:
            str_save = os.path.join(
                str_save_dir, f"{str_type}_{i_batch}_spatial_dog_fits.png"
            )
...
def plot_performance(d_track, model: Spatial_DoG, spatial_stas, str_save_dir=None):
    ...
    for i_cell in range(model.n_cells):
        r = np.corrcoef(
            spatial_stas[i_cell].cpu().numpy().flatten(), model_stas[i_cell].flatten()
        )[0, 1]
        ls_r.append(r)
...
def fit_spatial_dog(
    ...
    plot_spatial_dog_performance(
        model, spatial_stas, f"All", str_save_dir=str_save_dir, n_max_rows=50
    )
...
def rf_fitting_pipeline(
    stas: np.ndarray, str_output_dir: str, n_baseline_frames: int = 0
) -> dict:
    """
