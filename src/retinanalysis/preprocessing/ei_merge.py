...
def merge_eis(
    ...
    for id in cell_ids:
        eis = np.array(
            [
                all_eis[df][id].ei if id in all_eis[df] else np.zeros(ei_shape)
                for df in datafiles
            ]
        )
        ei_error = np.array(
            [
                all_eis[df][id].ei_error if id in all_eis[df] else np.zeros(ei_shape)
                for df in datafiles
            ]
        )
        n_spikes = np.array(
            [all_eis[df][id].n_spikes if id in all_eis[df] else 0 for df in datafiles]
        )
        nl_points = np.array(
            [
                all_eis[df][id].nl_points if id in all_eis[df] else sample_nl_points
                for df in datafiles
            ]
        )
        nr_points = np.array(
            [
                all_eis[df][id].nr_points if id in all_eis[df] else sample_nr_points
                for df in datafiles
            ]
        )

        # Make sure all EIs used the same number of time points
        assert np.all(nl_points == sample_nl_points), (
            "Not all eis have same number of left pts"
        )
        assert np.all(nr_points == sample_nr_points), (
            "Not all eis have same number of right pts"
        )
        ...
        # Calculate avg error
        avg_error = merge_error(avg_ei, total_spikes, n_spikes, ei_error, eis)

        combined_eis[int(id)] = WriteableEIData(
            ei_matrix=avg_ei, ei_error=avg_error, n_spikes=int(total_spikes)
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "exp_name", help="Name of experiment, (e.g. 20260224C)", type=str
    )
    parser.add_argument(
        "chunk_name", help="name of sorting chunk (e.g. chunk1)", type=str
    )
    parser.add_argument("datafiles", help="datafiles to include in chunk", nargs="*")

    # Optional arguments
    parser.add_argument(
        "--ss_version",
        help="version of kilosort used for spike sorting (e.g. kilosort2.5)",
        type=str,
        nargs="?",
        default="kilosort2.5",
        const="kilosort2.5",
