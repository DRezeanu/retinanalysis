...
class RawTraces:
    ...
    def load_bin_data(self, start_sample=0, end_sample=None, verbose=False):
        ...
        with bin2py.PyBinFileReader(
            ...
            if verbose:
                print(
                    f"Querying {query_samples} samples from {start_sample} to {end_sample}."
                )
                print(f"Queried time: {query_samples / SAMPLE_RATE} seconds")
                print(
                    f"From {start_sample / SAMPLE_RATE} to {end_sample / SAMPLE_RATE} seconds"
                )
            ...
            for start_idx in range(start_sample, end_sample, RW_BLOCKSIZE):
                ...
                data[
                    :,
                    start_idx - start_sample : start_idx
                    - start_sample
                    + n_samples_to_get,
                ] = chunk[1:, :]
    ...
    def load_epoch_index(self, epoch_idx, verbose=True):
        ...
        self.load_bin_data(
            start_sample=epoch_start, end_sample=epoch_end, verbose=verbose
        )
        self.epoch_idx = epoch_idx


def plot_sts_over_trace(
    rt: RawTraces,
    rb: MEAResponseBlock,
    cell_id,
    epoch_idx,
    ...
    for st in sts:
        ax.plot(
            time[st : st + n_highlight_width],
            raw_ts[st : st + n_highlight_width],
            color="red",
            lw=2,
            alpha=0.5,
        )
