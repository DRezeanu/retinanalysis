...
class PresentImagesSplitter:
    ...
    def split_responses_by_u_img(self):
        ...
        print(
            f"Using {imgs_per_epoch} images per epoch, {epoch_flash_frames.mean()} flash frames, {epoch_gap_frames.mean()} gap frames."
        )
        ...
        if len(repeats) > 1:
            raise NotImplementedError(
                f"Found images with different number of repeats: {repeats}. Please ensure all images have the same number of repeats."
            )
        repeats = repeats[0]
        ...
        if np.any(u_image_paths != self.sb.stim_frames["u_image_paths"]):
            raise ValueError(
                "u_image_paths in StimBlock does not match the u_image_paths generated from the epochs. Please check the StimBlock."
            )
        ...
        print(
            f"Average flash time: {avg_flash_time} s, gap time: {avg_gap_time} s, img samples: {avg_img_samples} samples"
        )
        ...
        for i in range(n_epochs):
            ...
            for j in range(imgs_per_epoch):
                t_onset = frame_times[i][
                    pre_frames - 1 + j * (flash_frames + gap_frames)
                ]
                t_offset = frame_times[i][
                    pre_frames - 1 + j * (flash_frames + gap_frames) + flash_frames
                ]
                t_p_end = frame_times[i][
                    pre_frames - 1 + (j + 1) * (flash_frames + gap_frames)
                ]
...
class ExpandingSpotsPipeline:
    def __init__(
        self, sb: StimBlock, rb: SCResponseBlock, b_sub_baseline: bool = False
    ):
        self.sb = sb
        self.rb = rb
    ...
    def get_stim_nspikes(self, b_sub_baseline: bool = False):
        ls_nsps = []
        print(
            "Getting stim spikes based on time, precise frametime not yet implement..."
        )
