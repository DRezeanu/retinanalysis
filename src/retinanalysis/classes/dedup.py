...
def compare_ei_methods(
    block: Union[AnalysisChunk, MEAResponseBlock], ei_threshold: float = 0.8, axs=None
):

    methods = ["full", "space", "power"]
    full_ei_autocorr, ei_full_pairs = get_ei_autocorrelation(
        block, ei_method=methods[0], ei_threshold=ei_threshold
    )
    space_ei_autocorr, ei_space_pairs = get_ei_autocorrelation(
        block, ei_method=methods[1], ei_threshold=ei_threshold
    )
    power_ei_autocorr, ei_power_pairs = get_ei_autocorrelation(
        block, ei_method=methods[2], ei_threshold=ei_threshold
    )
    ...
    venn3(
        [set(ei_full_pairs), set(ei_space_pairs), set(ei_power_pairs)],
        set_labels=methods,
        ax=axs[0, 3],
    )
    ...
    axs[1, 0].semilogy()
    axs[1, 0].axvline(
        full_thresh, color="red", linestyle="--", label=f"Threshold: {full_thresh:.2f}"
    )
    axs[1, 0].legend()
    ...
    axs[1, 1].axvline(
        ...
        color="red",
        linestyle="--",
        label=f"Threshold: {space_thresh:.2f}",
    )
    axs[1, 1].legend()
    ...
    axs[1, 2].axvline(
        ...
        color="red",
        linestyle="--",
        label=f"Threshold: {power_thresh:.2f}",
    )
    axs[1, 2].legend()
...
def get_sm_autocorrelation(ac: AnalysisChunk, sm_threshold: float):
    ...
    if not isinstance(ac, AnalysisChunk):
        raise ValueError(
            "Spatial map autocorrelation can only be computed for AnalysisChunk objects."
        )
    ...
    high_sm_pairs = set(
        [
            (ac.cell_ids[high_sm_idx[0][i]], ac.cell_ids[high_sm_idx[1][i]])
            for i in range(len(high_sm_idx[0]))
        ]
    )
    return sm_autocorr, high_sm_pairs


def get_ei_autocorrelation(
    block: Union[AnalysisChunk, MEAResponseBlock], ei_method, ei_threshold: float
):
    """
    ...
    high_ei_pairs = set(
        [
            (block.cell_ids[high_ei_idx[0][i]], block.cell_ids[high_ei_idx[1][i]])
            for i in range(len(high_ei_idx[0]))
        ]
    )
...
class DedupBlock:
    def __init__(
        ...
        is_noise: bool = False,
        ei_method: str = "full",
        ei_threshold: float = 0.80,
        sm_threshold: float = 0.80,
        **vu_kwargs,
    ):
        ...
        if is_noise:
            self.AnalysisChunk = AnalysisChunk(
                exp_name=exp_name,
                chunk_name=chunk_name,
                ss_version=ss_version,
                pkl_file=pkl_file,
                b_load_spatial_maps=True,
                **vu_kwargs,
            )
            self.cluster_to_index = dict(
                zip(
                    self.AnalysisChunk.cell_ids, range(len(self.AnalysisChunk.cell_ids))
                )
            )
        else:
            self.MEAResponseBlock = MEAResponseBlock(
                exp_name=exp_name,
            ...
            self.cluster_to_index = dict(
                zip(
                    self.MEAResponseBlock.cell_ids,
                    range(len(self.MEAResponseBlock.cell_ids)),
                )
            )
        ...
        if is_noise:
            ...
            self.sm_autocorr, high_sm_pairs = get_sm_autocorrelation(
                self.AnalysisChunk, sm_threshold=self.sm_threshold
            )
            self.ei_autocorr, high_ei_pairs = get_ei_autocorrelation(
                self.AnalysisChunk, self.ei_method, ei_threshold=self.ei_threshold
            )
            ...
            problem_cells, all_ei_cells, all_sm_cells = self.isolate_problem_cells(
                high_ei_pairs, high_sm_pairs
            )
        else:
            self.ei_autocorr, high_ei_pairs = get_ei_autocorrelation(
                self.MEAResponseBlock, self.ei_method, ei_threshold=self.ei_threshold
            )
            extended_ei_pairs = generate_extended_pairings(high_ei_pairs)
            problem_cells, all_ei_cells = self.isolate_problem_cells(high_ei_pairs)

        print("Creating deduplication sets...")
        self.dedup_sets["all_pairs"] = (
            high_ei_pairs.union(high_sm_pairs) if is_noise else high_ei_pairs
        )  # combined pairings
        self.dedup_sets["extended_pairs_across"] = generate_extended_pairings(
            self.dedup_sets["all_pairs"]
        )  # combined transitively connected groups across metrics
        self.dedup_sets["all_extended_pairs"] = (
            extended_ei_pairs.union(extended_sm_pairs)
            if is_noise
            else extended_ei_pairs
        )  # transitively connected groups within metrics

        self.dedup_sets["problem_cells"] = problem_cells  # all cells in any pair

        self.dedup_sets["ei_only_cells"] = problem_cells.difference(
            all_sm_cells
        )  # cells only in high ei pairs
        self.dedup_sets["ei_only_pairs"] = (
            self.dedup_sets["all_pairs"].difference(high_sm_pairs)
            if is_noise
            else high_ei_pairs
        )  # pairs only in high ei pairs
        self.dedup_sets["extended_ei_pairs"] = (
            extended_ei_pairs  # transitively connected groups within high ei pairs
        )
        if is_noise:
            self.dedup_sets["sm_only_cells"] = problem_cells.difference(
                all_ei_cells
            )  # cells only in high sm pairs
            self.dedup_sets["sm_only_pairs"] = self.dedup_sets["all_pairs"].difference(
                high_ei_pairs
            )  # pairs only in high sm pairs
            self.dedup_sets["extended_sm_pairs"] = extended_sm_pairs

            # now only the intersecting sets
            self.dedup_sets["both_cells"] = all_ei_cells.intersection(
                all_sm_cells
            )  # cells in both high ei and high sm pairs
            self.dedup_sets["both_pairs"] = high_ei_pairs.intersection(
                high_sm_pairs
            )  # pairs in both high ei and high sm pairs
            self.dedup_sets["extended_both_pairs"] = generate_extended_pairings(
                self.dedup_sets["both_pairs"]
            )  # transitively connected groups within both high ei and high sm pairs
    ...
    def isolate_problem_cells(self, high_ei_pairs: set, high_sm_pairs: set = None):
        ...
        if self.is_noise:
            problem_cells = {
                cell for pair in high_sm_pairs.union(high_ei_pairs) for cell in pair
            }
    ...
    def plotRFs_dedup(self, axs=None):
        ...
        for cell in cell_ids:
            ell1 = Ellipse(
                xy=(rf_params[cell]["center_x"], rf_params[cell]["center_y"]),
                width=rf_params[cell]["std_x"] * 2,
                height=rf_params[cell]["std_y"] * 2,
            ...
            if cell in self.dedup_sets["problem_cells"].difference(
                self.dedup_sets["ei_only_cells"]
            ) and cell in self.dedup_sets["problem_cells"].difference(
                self.dedup_sets["sm_only_cells"]
            ):
        ...
        custom_lines = [
            Line2D([0], [0], color="red", lw=1, label="Both"),
    ...
    def plot_correlations(self, axs=None):
        ...
        if self.is_noise:
            ...
            axs[1, 0].hist(
                np.triu(self.ei_autocorr, k=1).flatten(),
                bins=50,
                color="blue",
                alpha=0.7,
            )
            ...
            axs[1, 0].axvline(
                ...
                color="red",
                linestyle="--",
                label=f"Threshold: {self.ei_threshold}",
            )
            axs[1, 0].legend()
            axs[1, 1].hist(
                np.triu(self.sm_autocorr, k=1).flatten(),
                bins=50,
                color="green",
                alpha=0.7,
            )
            ...
            axs[1, 1].axvline(
                ...
                color="red",
                linestyle="--",
                label=f"Threshold: {self.sm_threshold}",
            )
            axs[1, 1].legend()
        else:
            if axs is None:
            ...
            axs[1].hist(
                np.triu_indices_from(self.ei_autocorr, k=1).flatten(),
                bins=50,
                color="blue",
                alpha=0.7,
            )
    ...
    def visualize_groups(self, group: tuple, axs=None, detailed=False):
        ...
        for i, cell in enumerate(group):
            if self.is_noise:
                ...
                if detailed:
                    ...
                    sorted_electrodes = eu.sort_electrode_map(
                        block.vcd.get_electrode_map()
                    )
                    ei = eu.reshape_ei(ei, sorted_electrodes)
                    ei = np.log10(
                        np.max(np.abs(ei), axis=2) + 1e-6
        ...
        ei_corrs = self.ei_autocorr[
            np.ix_(
                [self.cluster_to_index[c] for c in group],
                [self.cluster_to_index[c] for c in group],
            )
        ]
        if self.is_noise:
            sm_corrs = self.sm_autocorr[
                np.ix_(
                    [self.cluster_to_index[c] for c in group],
                    [self.cluster_to_index[c] for c in group],
                )
        ...
        fig.suptitle(
            f"Average EI correlation: {np.mean(ei_corrs):.2f}, Average SM correlation: {np.mean(sm_corrs):.2f}"
            if self.is_noise
            else f"Average EI correlation: {np.mean(ei_corrs):.2f}",
            fontsize=16,
        )
        plt.tight_layout()
        plt.subplots_adjust(top=0.95)
    ...
    def plot_amplitude_histograms(self, group: tuple, axs=None):
        ...
        for i, cell in enumerate(group):
            ...
            times = (
                self.amplitudes[2, self.amplitudes[1, :] == cell] * 1000 / 20000
            )  # convert to seconds assuming 20kHz sampling rate
    ...
    def get_summary_stats(self, pairs: set):
        ...
        for a, b in pairs:
            if self.is_noise:
                sm_corr = self.sm_autocorr[
                    self.cluster_to_index[a], self.cluster_to_index[b]
                ]
            ei_corr = self.ei_autocorr[
                self.cluster_to_index[a], self.cluster_to_index[b]
            ]
            overlap_fraction = self.get_amplitude_overlap((a, b))
            stats.append(
                [a, b, ei_corr, sm_corr if self.is_noise else None, overlap_fraction]
            )
