...
def load_and_process_img(
    str_img,
    screen_size=np.array([1140, 1824]),  # rows, cols
    ...
    img_resized = cv2.resize(
        img, tuple(scene_size[::-1]), interpolation=cv2.INTER_LINEAR
    )
...
def get_image_paths_across_epochs(
    df_epochs: pd.DataFrame, n_actual_imgs_per_epoch: Optional[int] = None
):
    ...
    assert np.all(np.array(num_imgs_across_epochs) == num_imgs_across_epochs[0]), (
        "Not all epochs have same number of images!"
    )
    assert np.all(
        np.array(num_folders_across_epochs) == num_folders_across_epochs[0]
    ), "Not all epochs have same number of folders!"
    assert num_imgs_across_epochs[0] == num_folders_across_epochs[0], (
        "Number of image names and folders do not match!"
    )
...
def get_old_presentimages_transitions(
    df_epochs: pd.DataFrame, frame_rate=59, stride=1, pre_flash_time=0.25, verbose=True
):
    ...
    stim_bins = (
        (state_time >= pre_time) & (state_time < pre_time + stim_time)
    ).sum() * stride
    ...
    for i in range(n_trials):
        for j in range(n_flashes):
            flash_bins = (
                ...
                & (state_time <= (j * (flash_time + gap_time) + flash_time))
            ).sum()
            flash_bins = flash_bins * stride
            gap_bins = (
                (state_time >= (j * (flash_time + gap_time) + flash_time))
                & (state_time < (j + 1) * (flash_time + gap_time))
            ).sum()
    ...
    if verbose:
        ...
        print(f"Pre bins: {pre_bins}, Stim bins: {stim_bins}, Tail bins: {tail_bins}")
        print(f"Total bins: {total_bins}")
        print(f"Unique flash bins: {np.unique(ls_flash_bins, return_counts=True)}")
...
def compute_image_onset_offset_frames(
    n_epochs, imgs_per_epoch, pre_frames, epoch_flash_frames, epoch_gap_frames
):
    ...
    for i in range(n_epochs):
        ...
        for j in range(imgs_per_epoch - 1):
            ...
            img_offset_frames[i, j + 1] = (
                img_onset_frames[i, j + 1] + epoch_flash_frames[i, j + 1]
            )

    return img_onset_frames, img_offset_frames


def get_present_images_transitions(
    df_epochs: pd.DataFrame, rb: MEAResponseBlock | SCResponseBlock
):
    ...
    if len(np.unique(stage_frame_rates)) > 1:
        raise ValueError(
            f"Multiple stage frame rates found: {np.unique(stage_frame_rates)}. Please ensure all epochs have the same stage_frame_rate."
        )
    else:
    ...
    if "flashFrames" not in df_epochs.at[0, "epoch_parameters"].keys():
        ...
        d_bins = get_old_presentimages_transitions(
            df_epochs, frame_rate=stage_frame_rate, stride=1, verbose=True
        )
        ...
        epoch_flash_frames = np.repeat(
            np.array(epoch_flash_frames)[:, np.newaxis], imgs_per_epoch, axis=1
        )
        epoch_gap_frames = np.repeat(
            np.array(epoch_gap_frames)[:, np.newaxis], imgs_per_epoch, axis=1
        )
    ...
    print(
        f"Protocol had {imgs_per_epoch} images per epoch, {np.unique(epoch_flash_frames)} flash frames, {np.unique(epoch_gap_frames)} gap frames."
    )
    ...
    for i in range(n_epochs):
        ...
        n_expected_stim_frames = np.floor(stim_time / 1e3 * stage_frame_rate).astype(
            int
        )
        ...
        if n_frames < n_expected_stim_frames:
            raise ValueError(
                f"Epoch {i} had only {n_frames} frames, but expected {n_expected_stim_frames} frames for {stim_time} s stimulus time at {stage_frame_rate} Hz stage frame rate."
            )
        ...
        if last_img_offset > n_expected_stim_frames:
            ...
            last_img_idx = np.where(img_offset_frames[i] <= n_expected_stim_frames)[0][
                -1
            ]
            ...
            print(
                f"But last image offset frame {last_img_offset} exceeds {n_expected_stim_frames} frames."
            )
            print(f"Epoch {i} thus only showed {imgs_per_epoch} images.")
            print(
                f"Assuming {imgs_per_epoch} is the correct number of images per epoch for all epochs.\n"
            )
    ...
    all_image_paths = get_image_paths_across_epochs(
        df_epochs, n_actual_imgs_per_epoch=imgs_per_epoch
    )
...
def load_all_present_images(
    ...
    mag_factors = [
        row["epoch_parameters"]["magnificationFactor"]
        for _, row in df_epochs.iterrows()
    ]
    u_mag_factors = np.unique(mag_factors)
    if len(u_mag_factors) > 1:
        raise ValueError(
            f"Multiple magnification factors found: {u_mag_factors}. Please ensure all epochs have the same magnificationFactor."
        )
    ...
    d_display_params["ds_pix"] = int(
        d_display_params["ds_mu"] / d_display_params["mu_per_pix"]
    )
    d_display_params["ds_screen_size"] = (
        d_display_params["screen_size"] / d_display_params["ds_pix"]
    ).astype(int)

    print(
        f"Using {ds_mu} um pixel size, {d_display_params['ds_pix']} pixels per um, "
...
def make_variable_mean_bars(
    ...
    startDims = matlab.double(
        [df_epochs.loc[i, "epoch_parameters"]["startDim"] for i in df_epochs.index]
    )
    trackEnds = matlab.double(
        [df_epochs.loc[i, "epoch_parameters"]["trackEnd"] for i in df_epochs.index]
    )
    ...
    if b_noise_only:
        ...
        low_means = matlab.double(
            [df_epochs.loc[i, "epoch_parameters"]["lowMean"] for i in df_epochs.index]
        )
        high_means = matlab.double(
            [df_epochs.loc[i, "epoch_parameters"]["highMean"] for i in df_epochs.index]
        )
        print(
            f"Using epoch low/high means for regen, first epoch: {low_means[0]}, {high_means[0]}"
        )
    numChecksYs = matlab.double(
        [df_epochs["epoch_parameters"][i]["numChecksY"] for i in df_epochs.index]
    )
...
def make_bars_and_gain(
    ...
    low_means = matlab.double(
        [
            df_epochs.loc[i, "epoch_parameters"]["backgroundIntensity"]
            for i in df_epochs.index
        ]
    )
    high_means = matlab.double(
    ...
    backgroundFrameDwells = matlab.double([1000 for _ in df_epochs.index])
...
def regenerate_projector_gain(df_epochs, str_pkg_dir):
    import matlab.engine  # type: ignore
    ...
    stepDurations_arr = []
    for epoch_idx in range(len(df_epochs)):
        gains_epoch = matlab.double(
            [
                df_epochs.loc[epoch_idx, "epoch_parameters"]["projGainMeans"][i]
                for i in range(
                    len(df_epochs.loc[epoch_idx, "epoch_parameters"]["projGainMeans"])
                )
            ]
        )
        stepDurations = matlab.double(
            [
                df_epochs.loc[epoch_idx, "epoch_parameters"]["projStepDurations"][i]
                for i in range(
                    len(
                        df_epochs.loc[epoch_idx, "epoch_parameters"][
                            "projStepDurations"
                        ]
                    )
                )
        ...
        gainValues_arr.append(gains_epoch)
        stepDurations_arr.append(stepDurations)

    upperLimit = matlab.double(1)
    lowerLimit = matlab.double(0)
    sampleRate = matlab.double(df_epochs.loc[0, "epoch_parameters"]["sampleRate"])

    gain_trace = eng.util.regenerateProjectorGain(
        preTime,
        stimTime,
        tailTime,
        stepDurations_arr,
...
def make_doves_perturbation_alpha(
    ...
    for e_idx in range(n_epochs):
        ...
        steps_per_stixel = np.max(
            [np.round(stixel_size_pix / grid_size_pix), 1]
        ).astype(int)
        stixel_shift_pix = np.round(stixel_size_pix / steps_per_stixel).astype(int)
        print(
            f"Stixel size: {stixel_size_um} um, grid size: {grid_size_um} um, steps per stixel: {steps_per_stixel}, stixel shift: {stixel_shift_pix} pix"
        )
        ...
        noise_lines = cv2.resize(
            noise_lines, upsample_size[::-1], interpolation=cv2.INTER_NEAREST
        )
        ...
        for f_count in range(2, n_frames + 1):
            if f_count % frame_dwell == 0:
                x_shift_stix = np.round(
                    np.random.rand() * (steps_per_stixel - 1)
                ).astype(int)
                ...
                if x_shift_stix == 0:
                    continue
                noise_lines[x_shift_stix:, f_count - 1] = noise_lines[
                    :-x_shift_stix, f_count - 1
                ]
...
def make_expanding_spots(df_epochs: pd.DataFrame, ds_mu: float = 10.0):
    ...
    d_output = {"image_data": all_images, "d_display_params": d_display_params}
...
def make_single_doves_movie(
    ...
    img = cv2.resize(
        img, tuple(scene_size[::-1].astype(int)), interpolation=cv2.INTER_NEAREST
    )
    ...
    if verbose:
        print(
            f"Creating movie for {imageName} with shape {img.shape} and scene size {scene_size}, fitting in {screen_size}"
        )
    if b_ds_to_stix:
    ...
    if verbose:
        print(
            f"Adding {wait_frames} wait frames to {frames.shape} for {wait_time_s} s."
        )
    ...
    if verbose:
        print(
            f"Movie scaled to (-1, 1) contrast: min {frames.min():.2f}, max {frames.max():.2f}"
        )
...
def make_all_doves_movies(
    ...
    D_DOVES_DATA = loadmat(
        os.path.join(
            str_manookin_pkg_dir, "resources", "doves", "dovesFEMstims20160826.mat"
        )
    )
    STR_DOVES_IMG_DIR = os.path.join(
        str_manookin_pkg_dir, "resources", "doves", "images"
    )
    ...
    if len(np.unique(frame_counts)) == 1:
        print(
            f"All movies have same frame count of {frame_counts[0]}, stacking into array."
        )
