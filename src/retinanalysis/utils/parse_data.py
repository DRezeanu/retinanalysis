...
def get_litke_triggers(bin_path, RW_BLOCKSIZE=2000000, TTL_THRESHOLD=-1000):
    ...
    try:
        with bin2py.PyBinFileReader(
            bin_path, chunk_samples=RW_BLOCKSIZE, is_row_major=True
        ) as pbfr:
            ...
            for start_idx in range(0, n_samples, RW_BLOCKSIZE):
                ...
                above_to_below_threshold = np.logical_and.reduce(
                    [above_threshold[:-1], below_threshold[1:]]
                )
...
def calculate_up_down_offsets(
    frame_ups: np.ndarray, frame_downs: np.ndarray, expected_frame_rate: float = 59.94
) -> float:
    ...
    try:
        ...
        delta_down = np.round(np.mean(d_down) - 1000.0 / expected_frame_rate).astype(
            int
        )
...
def upsample_frame_times(ttl_times: np.ndarray, frames_per_ttl: int) -> np.ndarray:
    ...
    for i in range(ttl_times.shape[0] - 1):
        frame_times.append(
            np.linspace(ttl_times[i], ttl_times[i + 1], frames_per_ttl, endpoint=False)
        )
    return np.asarray(frame_times).ravel()


def find_optimal_threshold(
    f_monitor: np.ndarray, sample_rate: float = 10000.0
) -> float:
    ...
    for t_idx, threshold in enumerate(thresholds):
        ...
        frame_violations[t_idx] = len(
            np.where(d_frames / sample_rate * 1000.0 < 32.0)[0]
        )
    return np.max(
        thresholds[np.where(frame_violations == np.min(frame_violations))]
    ).astype(float)


def check_frame_times(
    frame_times: np.ndarray, expected_frame_rate: float = 60.0, bin_rate: float = 1000.0
):
...
def get_frame_times(
    frame_monitor, bin_rate: float = 1000.0, sample_rate: float = 10000.0
):
...
def get_frame_times_lightcrafter(
    ...
    try:
        ...
        frame_ups = find_threshold_cross(f_monitor, threshold=threshold, direction=1)
        frame_downs = find_threshold_cross(f_monitor, threshold=threshold, direction=-1)
        delta_down = calculate_up_down_offsets(
            frame_ups, frame_downs, expected_frame_rate=59.94
        )
        ...
        frame_times = np.round(
            np.arange(0.0, len(f_monitor) / sample_rate * bin_rate, frame_step)
        ).astype(float)
    return frame_times


def get_frame_times_pattern_mode(
    frame_monitor: np.ndarray,
    bin_rate: float = 1000.0,
    sample_rate: float = 10000.0,
...
def get_frame_times_from_syncs(
    sync: np.ndarray, bin_rate: float = 1000.0, sample_rate: float = 10000.0
):
    sync -= np.min(sync)
    sync /= np.max(sync)
...
def get_frame_times_from_pwm(
    pwm: np.ndarray, bin_rate: float = 1000.0, sample_rate: float = 10000.0
):
    d_pwm = np.diff(pwm)
    ...
    def __init__(self, d: Optional[dict] = None) -> None:
        ...
        if d is not None:
            ...
            if "creationTimeDotNetDateTimeOffsetTicks" in d["attributes"].keys():
                creationTimeDotNetDateTimeOffsetTicks = d["attributes"][
                    "creationTimeDotNetDateTimeOffsetTicks"
                ]
                self.start_time = dotnet_ticks_to_datetime(
                    creationTimeDotNetDateTimeOffsetTicks
                )[0]
            elif "startTimeDotNetDateTimeOffsetTicks" in d["attributes"].keys():
                startTimeDotNetDateTimeOffsetTicks = d["attributes"][
...
class ExperimentObj(SourceObj):
    def __init__(
        self, d: Optional[dict] = None, rig_type: Optional[str] = None
    ) -> None:
        super().__init__(d=d)
    ...
    def parse(self, d: Optional[dict]):
        if d is not None:
            if "endTimeDotNetDateTimeOffsetTicks" in d["attributes"].keys():
                self.set_end_time_from_ticks(
                    d["attributes"]["endTimeDotNetDateTimeOffsetTicks"]
                )
            if "block" in d.keys():
...
class DataObj(object):
    ...
    def parse_dict(self, d: Optional[dict]):
        if d is not None:
            ...
            if "endTimeDotNetDateTimeOffsetTicks" in d["attributes"].keys():
                endTimeDotNetDateTimeOffsetTicks = d["attributes"][
                    "endTimeDotNetDateTimeOffsetTicks"
                ]
                self.end_time = dotnet_ticks_to_datetime(
                    endTimeDotNetDateTimeOffsetTicks
                )[0]
            # Set the attributes.
            self.parse_attributes(d=d["attributes"])
...
class ResponseObj(object):
    ...
    def set_from_dict(self, label: str, d: dict):
        ...
        self.inputTimeDotNetDateTimeOffsetOffsetHours = d[
            "inputTimeDotNetDateTimeOffsetOffsetHours"
        ]
        self.inputTimeDotNetDateTimeOffsetTicks = d[
...
class StageObj(object):
    def __init__(
        self, key_name: str, properties: dict, frame_times_from_syncs: bool = True
    ) -> None:
    ...
    def parse_properties(self, key_name: str, properties: dict):
        ...
        if "LightCrafter" in key_name:
            ...
            try:
                if ("lightCrafterPatternRate" in properties.keys()) and (
                    "monitorRefreshRate" in properties.keys()
                ):
    ...
    def get_frame_times_lcr_pattern_mode(self):
        ...
        return get_frame_times_pattern_mode(
            self.frame_data,
            bin_rate=1000.0,
    ...
    def get_frame_times_lcr_video_mode(self):
        if self.frame_data is None:
            ...
            return np.array([])
        assert self.sample_rate is not None, "self.sample_rate is None, cannot proceed"
        return get_frame_times_lightcrafter(
            self.frame_data, bin_rate=1000.0, sample_rate=self.sample_rate
        )
    ...
    def get_frame_times(self):
        if not self.frame_times_from_syncs:
            ...
            assert self.frame_rate is not None, (
                "self.frame_rate is None, cannot proceed"
            )
            ...
            assert self.frame_data is not None, (
                "self.frame_data is None, cannot proceed"
            )
            print("Frame data length: {}".format(len(self.frame_data)))
            assert self.sample_rate is not None, (
                "self.sample_rate is None, cannot proceed"
            )
            frame_times = np.round(
                np.arange(
                    0.0, len(self.frame_data) / self.sample_rate * bin_rate, frame_step
                )
            ).astype(float)
            return frame_times
    ...
    def parse_backgrounds(self, d: h5py.Group):
        ...
        for key, value in d["backgrounds"].items():  # type: ignore
            ...
            if "dataConfigurationSpans" in value.keys():
                try:
                    foo = parse_attributes(
                        value["dataConfigurationSpans"][
                            list(value["dataConfigurationSpans"].keys())[0]
                        ][key_name]
                    )
...
class FrameObj(object):
    def __init__(self, stage_obj: StageObj, frame_monitor: np.ndarray) -> None:
        pass
...
class Symphony2Reader:
    ...
    def read_write(self, datajoint: bool = False):
        ...
        if self.json_path is not None:
            ...
            if self.mea_raw_data_path is not None:
                self.export_summary_text(
                    experiment, self.json_path.replace(".json", ".txt")
                )
    ...
    def create_symphony_dict(self, metadata: dict) -> dict:
        ...
        for hh in range(len(exp_animals)):
            ...
            for ii in range(len(exp_preparations)):
                ...
                if not cells:
                    raise ValueError(
                        "Cells level is empty, only Preparation and Animal found"
                    )
                for jj in range(len(cells)):
                    ...
                    for kk in range(len(metadata["group"])):
                        if cell["uuid"] == metadata["group"][kk]["source"]["uuid"]:
                            ...
                            this_array_pitch = self.array_pitch_dict[
                                metadata["group"][kk]["uuid"]
                            ]
    ...
    def organize_metadata(self):
        ...
        for group in self.metadata["group"]:
            ...
            for g_label in group_protocols:
                ...
                for block in group["block"]:
                    p_label = block["protocolID"]
                    if (self.mea_raw_data_path is not None) and (
                        "dataFile" in block.keys()
                    ):
    ...
    def export_summary_text(self, experiment: dict, filepath: str):
        """
                        ...
                        for group in cell["epoch_groups"]:
                            ...
                            for block in group["epoch_blocks"]:
                                ...
                                if match:
                                    ...
                                    print(
                                        f"Warning: no numbers found in filename: {f_name}"
                                    )
        ...
        for key, value in responses.items():
            ...
            if ("Sync" in key) and ("data" in value.keys()):
                ...
                if self.current_epoch.stage_obj is None:
                    self.current_epoch.stage_obj = StageObj(
                        key_name=key_name,
                        properties=response_dict[key_name],
                        frame_times_from_syncs=self.frame_times_from_syncs,
                    )
    ...
    def parse_epoch_block(self, epoch_block: dict) -> Optional[dict]:
        ...
        if self.mea_raw_data_path is not None:
            if "dataFileName" not in block_obj.properties.keys():
                ...
                assert self.experiment_name is not None, "self.experiment_name is None"
                f_path = os.path.join(
                    self.mea_raw_data_path, self.experiment_name, f_name.split("/")[-2]
                )
                ...
                if int(self.experiment_name[:8]) < 20220518:
                    litke_starts, litke_ends, array_id, n_samples = (
                        get_litke_triggers_old(f_path)
                    )
                else:
                    litke_starts, litke_ends, array_id, n_samples = get_litke_triggers(
                        f_path
                    )

                if array_id is None or n_samples is None:
                    print(
                        f"ERROR: Unable to parse data file {block_obj.__dict__['properties']['dataFileName']}, see error on previous line.\n"
                    )
                    return None
                else:
                    self.array_pitch = get_electrode_pitch_by_array_id(
                        array_id=array_id
                    )
                    # Check for unrecorded epochs.
                    if np.any(np.array(litke_starts) > n_samples):
                        unrecorded_epochs = np.argwhere(
                            np.array(litke_starts) > n_samples
                        ).ravel()
        ...
        for count, epoch in enumerate(epoch_list):
            epoch_starts[count] = epoch["attributes"][
                "startTimeDotNetDateTimeOffsetTicks"
            ]
        ...
        if self.mea_raw_data_path is not None:
            if len(litke_starts) == 0:
                ...
                litke_starts = 46969 + np.floor(
                    (start_seconds - start_seconds[0]) * self.sample_rate
                ).astype(int)
            ...
            if len(epoch_starts) < len(litke_starts):
                start_samples = litke_starts[0] + np.floor(
                    (start_seconds - start_seconds[0]) * self.sample_rate
                ).astype(int)
                ...
                if len(start_samples) > 1:
                    ...
                    if np.any(dt > 0.5 * d_samps):
                        print(
                            "WARNING: Found missing epochs start pulses in file: "
                            + f_name
                        )
                        litke_starts = start_samples[:n_epochs]
            if len(epoch_starts) > len(litke_starts):
                print(
                    f"WARNING: More epochs ({len(epoch_starts)}) than Litke triggers ({len(litke_starts)}) found in file: "
                    + f_name
                )
    ...
    def parse_epoch_group(self, epoch_group: h5py.Group) -> dict:
        ...
        for key, value in epoch_group["epochBlocks"].items():  # type: ignore
            ...
            if this_block is not None:
                block_start_times.append(
                    this_block["attributes"]["startTimeDotNetDateTimeOffsetTicks"]
                )
    ...
    def parse_file(self, exp_key) -> dict:
        ...
        for _, value in sources.items():  # type: ignore
            ...
            for _, g_value in exp_groups.items():
                print(
                    "Parsing group {}".format(group_count + 1)
                    + " of {}...".format(len(exp_groups))
                )
                this_group = self.parse_epoch_group(g_value)
                group_list.append(this_group)
                group_start_times.append(
                    this_group["attributes"]["startTimeDotNetDateTimeOffsetTicks"]
                )
                group_count += 1
...
def h5_to_datajoint_json(
    h5_path: str,
    output_directory: str = "/Volumes/data-1/datajoint/mea/meta/",
    ...
    **kwargs,
    ...
    reader = Symphony2Reader(
        h5_path=h5_path, out_path=out_path, mea_raw_data_path=raw_directory, **kwargs
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Parse H5 experiment files into JSON format."
    )
    parser.add_argument(
        "h5_path",
        type=str,
        help="Path to the Symphony HDF5 file (e.g., /data/data/h5/20230607C.h5).",
    )
    parser.add_argument(
        "out_path",
        type=str,
        help="Output path for the JSON and TXT files (e.g., /data/data/metadata/json/20230607C.json).",
    )
    parser.add_argument(
        "-r",
        "--raw_data_path",
    ...
    parser.add_argument(
        "-f",
        "--frame_times_from_syncs",
        action="store_true",
        help="Whether to extract frame times from syncs (default: False).",
    )
    ...
    with Symphony2Reader(
        ...
        frame_times_from_syncs=frame_times_from_syncs,
